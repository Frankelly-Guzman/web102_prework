...
function addGamesToPage(games) {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    gameCard.innerHTML = `
        ...
        <p>Pledged: $${games[i].pledged} of $${games[i].goal} goal</p>
...
let sumOfUnfundedGamesCount = GAMES_JSON.filter((game) => {
  return game.goal > game.pledged;
}).length;

let sumOfGames = GAMES_JSON.length;
// create a string that explains the number of unfunded games using the ternary operator
let descriptionText = `A total of $${totalRaised.toLocaleString(
  "en-US"
)} has been raised for ${sumOfGames} games. Currently, ${sumOfUnfundedGamesCount} games remain 
